...
/*
    ...
    esse operador é usado para selecionar os dados que são iguais ao que foi passado no select, sendo não sensível a maiuscula/minuscula  
*/

SELECT * FROM "produtos" WHERE "nome" LIKE 'impressora 3d mini';
-- até esse momento, como se fosse o COLLATE NOCASE
...
/*
    ...
    '%pão' = seleciona strings que possuem caracteres antes de pão e finaliza com pão
    'pão%' = seleciona strings que possuem caracteres depois de pão e inicia com pão
    '%pão%' = seleciona strings que possuem 'pão' em alguma parte da string
*/

SELECT * FROM "produtos" WHERE "nome" LIKE 'Processador%';

SELECT * FROM "produtos" WHERE "nome" LIKE '%th';
...
/*
    ...
    _
    _ : Representa exatamente 1 caractere não especificado.

    '_pão' = seleciona strings que possui 1 caractere antes de pão e finaliza com pão
    'pão_' = seleciona strings que possui 1 caractere depois de pão e inicia com pão
    '_pão_' = seleciona strings que possui 1 caractere antes e depois de 'pão', e somente esses caracteres adicionais


    é possível combinar % e _:

    exemplo:

    '%_pão%' = seleciona strings que possui 1 caractere antes de pão pelo menos e que pão esteja presente em alguma parte da string
...
/*
    ...
    O NOT LIKE basicamente funciona de forma contrária ao LIKE, ele irá selecionar todos os registros que não possui padrões em comum ao que foi passado no NOT LIKE

    NOT LIKE '%pão%' - seleciona tudo que não tem pão em algum momento da string
