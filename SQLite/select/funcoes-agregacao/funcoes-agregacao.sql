...
/*
    Existem nativas de um banco de dados SQL.

    Um dos tipos são as funções de agregação:

    funções de agregação são funções matemáticas.
...
SELECT COUNT(*) FROM "produtos" WHERE "nome" LIKE '%processador%';
SELECT SUM("preco") FROM "produtos" WHERE "nome" LIKE '%processador%';

SELECT AVG("preco") FROM "produtos";
-- aqui retorna a média exata.
-- Caso eu queira arredondar o valor, eu posso usar a função ROUND()
...
-- posso mudar o nome de uma seleção de campo usando AS:

SELECT nome, MAX("preco") AS "preco_produto_mais_caro" FROM "produtos";
