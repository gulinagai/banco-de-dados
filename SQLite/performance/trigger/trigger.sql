...
-- Trigger
-- Os triggers são utilizados para executar ações automaticamente quando um evento ocorre em uma tabela.

-- um evento é um insert, update ou delete.
...
-- através das palavras chaves: NEW - para o novo dado e OLD para o dado antigo

-- no update: temos acesso ao NEW e ao OLD
-- no insert: temos acesso ao NEW somente, já que não havia dado antigo
-- no delete: temos acesso ao OLD somente, já que não haverá um dado novo
